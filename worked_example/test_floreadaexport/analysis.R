parentdir <- '/Users/sophia01px2019/Downloads/test_floreadaexport'
floreadaexportdir <- file.path(parentdir, 'floreadexport_092925')
...
bfpdt <- fread( file.path( floreadaexportdir, 'BFP_combodf.txt') ) %>%
   filter( `RL1-A` > 0 & `VL1-A` > 0 & `YL1-A` > 0 & `BL1-A` > 0 ) # remove zero signal events

pc3dt <- fread( file.path( floreadaexportdir, 'PC3_combodf.txt')) %>%
   filter( `RL1-A` > 0 & `VL1-A` > 0 & `YL1-A` > 0 & `BL1-A` > 0) # remove zero signal events
...
bfpdt <- dplyr::mutate( bfpdt, log10_RL1A = log10(`RL1-A`),
                        ...
                        log10_FSC = log10(`FSC-A`),
                        log10_SSC = log10(`SSC-A`),
